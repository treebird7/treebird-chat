...
export const FLAT_RE = /^\[(\d{2}:\d{2})\s+([A-Za-z][A-Za-z0-9_-]*)\]\s?(.*)$/;
...
export function findCursorAfterLastSelfRound(lines, agent) {
  ...
  const flatRe  = new RegExp(`^\\[\\d{2}:\\d{2}\\s+${escapeRe(agent)}\\]`, 'i');
...
export function diffSinceBaseline(filePath, baseline, endWord, agent = null) {
  ...
  const selfFlatRe = agent ? new RegExp(`^\\[\\d{2}:\\d{2}\\s+${escapeRe(agent)}\\]`, 'i') : null;
