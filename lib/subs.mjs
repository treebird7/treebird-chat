...
function autoSummary(subFile) {
  ...
  try {
    ...
    // Frozen format (optional date + agent#N) — groups: 1 date, 2 time, 3 agent, 4 instance, 5 text.
    const FLAT_RE = /^\[(?:(\d{4}-\d{2}-\d{2}) )?(\d{2}:\d{2}) ([^\]#]+?)(?:#(\d+))?\] ?(.*)$/;
    // Last 3 protocol lines, joined with " · "
    const protocol = lines.filter(l => FLAT_RE.test(l)).slice(-3);
    if (!protocol.length) return '';
    return protocol
      .map(l => { const m = FLAT_RE.exec(l); return m ? `${m[3].trim()}${m[4] ? '#' + m[4] : ''}: ${m[5].slice(0, 60)}` : ''; })
      .filter(Boolean)
      .join(' · ');
  } catch {
    return '';
