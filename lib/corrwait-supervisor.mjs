...
export async function supervise(opts) {
  ...
  // Merge caller's extraArgs so filters like --on-mention apply on catchup
  // too — otherwise a restarting mention-only agent gets woken by the whole
  // backlog of unmentioned lines.
  if (catchup) {
    const { payload: cu } = await runOnce({
      corrwaitBin, filePath, agent, timeoutSec,
      extraArgs: [...extraArgs, '--catchup'],
      stderrPassthrough,
    });
