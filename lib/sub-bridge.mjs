...
// regex bin/treebird-chat-join.mjs:41 enforces on its argv). 64-char cap
// matches AGENT_NAME_RE's ceiling — defense against an oversized topic
// reaching this lib via a direct API caller (the TUI's /sub already
// rejects > 48 chars, but lib must defend itself). (/ts-review #6.)
const TOPIC_MAX_LEN = 64;
function sanitiseTopic(topic) {
  return String(topic).replace(/[^A-Za-z0-9_-]/g, '-').slice(0, TOPIC_MAX_LEN);
}
...
export async function spawnSubBridge({ parentFile, subFile, subTopic, agent }) {
  ...
  // Attach an 'error' handler — without one, async spawn failures (ENOENT
  // on node binary, EACCES on the bridge script) become unhandled exceptions
  // that crash the parent TUI. The liveness probe below catches the
  // resulting dead PID and reports it cleanly. (/ts-review spawn_hygiene.)
  child.on('error', (e) => process.stderr.write(`[sub-bridge] spawn error: ${e.message}\n`));
  child.unref();
