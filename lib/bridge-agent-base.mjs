...
const CORRWAIT_TIMEOUT_MS = 600_000; // 10 min safety margin over corrwait's 540s

function runCorrwait(corrwaitBin, filePath, agent) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [corrwaitBin, filePath, '--as', agent, '--timeout', '540'], {
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: CORRWAIT_TIMEOUT_MS,
      // Minimal env: corrwait only needs its identity. BIRDCHAT_AGENT is passed
      // explicitly; ENVOAK_AGENT_LABEL is deliberately omitted so it can't
      // override the bridge's chosen agent name.
...
export async function runAgentBridge({ agentLabel, filePath, contextLines = 30, callLLM, corrwaitBin, log }) {
  ...
  while (true) {
    const result = await runCorrwait(corrwaitBin, filePath, agentLabel);
    ...
    if (result.reason === 'ERROR') {
      logger('corrwait exited with error — backing off 10s before retry');
