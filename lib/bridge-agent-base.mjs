...
export async function runAgentBridge({ agentLabel, filePath, contextLines = 30, callLLM, corrwaitBin, log, selectTrigger }) {
  ...
  const result = await supervise({
    ...
    log: logger,
    // --raw: this bridge scans payload.newContent for @mentions, so it needs the
    // raw new lines. corrwait's default is lean (wakeLines only); opt back in here.
    extraArgs: ['--raw'],
    ...
    onWake: async (payload) => {
      ...
      // Trigger seam (obsidigem SPEC §2.2): a caller may supply selectTrigger to
      // redefine "what wakes me" (e.g. all-traffic mode reading a config sidecar).
      // Default = mention-only, so gemma-bridge and every existing caller are unchanged.
      const mentions = selectTrigger
        ? selectTrigger(newLines, agentLabel)
        : scanForMentions(newLines, agentLabel, 0).mentions;
