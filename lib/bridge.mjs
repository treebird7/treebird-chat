...
async function watchFileLoop(context) {
  for await (const { lineNo, line } of context.archive.watchForNewLines(
    ...
    if (context.state.selfInsertedLines.has(lineNo) || context.selfInsertedContent.has(line)) {
      context.state.selfInsertedLines.delete(lineNo);
      context.selfInsertedContent.delete(line);
      await context.persist();
      continue;
    }
...
export async function runBridge({ chatId, file, transport, archive, cursorStore, signal }) {
  ...
  const context = {
    ...
    readReady,
    pendingWrites,
    selfInsertedContent: new Set(), // in-session content guard against watchFileLoop race
  };
