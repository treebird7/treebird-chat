...
import { MAX_LINE_LEN } from './writer.mjs';

const DEFAULT_POLL_MS = Number.parseInt(process.env.BIRDCHAT_BRIDGE_POLL_MS || '500', 10) || 500;
// Adaptive idle backoff: an active room polls at DEFAULT_POLL_MS for snappy
// chat, but a quiet room widens its interval toward this cap so ~30 idle/stale
// rooms don't each hammer the server at 2 req/s round the clock. Snaps back to
// fast the moment a sync returns any messages.
const DEFAULT_IDLE_POLL_MS = Number.parseInt(process.env.BIRDCHAT_BRIDGE_IDLE_POLL_MS || '5000', 10) || 5000;
...
async function syncOnce(context) {
  ...
  for (const message of messages) {
    ...
    // Inbound peer line exceeds local per-line cap. Substitute a visible system
    // note so other participants see *why* the message didn't land, rather than
    // silently dropping or rendering an oversize line that may break atomic
    // O_APPEND guarantees (PIPE_BUF). Source bridge should have caught this at
    // its own writer — that's a separate hardening on their side.
    if (line.length > MAX_LINE_LEN) {
      const oversize = line.length;
      // author is peer-controlled — cap to keep the substitute line itself
      // under MAX_LINE_LEN. The rest of the text is fixed-shape and small.
      const author = String(message.agent ?? 'unknown').slice(0, 80);
      const time = message.time ?? '';
      let note = encodeLine({
        agent: 'system',
        time,
        text: `⚠️ inbound message from ${author}${time ? ` at ${time}` : ''} exceeded line limit (${oversize} chars > ${MAX_LINE_LEN}) — not appended. Source should split into shorter posts.`,
      });
      // Defense in depth: if the substitute somehow still doesn't fit (caller
      // passed bizarre time, future code change inflated the message), fall
      // back to a fully static note that's known to fit.
      if (note.length > MAX_LINE_LEN) {
        note = encodeLine({
          agent: 'system',
          time: '00:00',
          text: `⚠️ inbound message exceeded line limit (${MAX_LINE_LEN}) — not appended.`,
        });
      }
      line = note;
    }
    // Skip lines already present in the file at startup (replay guard). One
    // credit per pre-existing line so identical content isn't over-suppressed.
    if (context.existingFileContent.take(line)) {
      context.state.lastSmalltoakId = Math.max(context.state.lastSmalltoakId, message.id);
      await context.persist();
      continue;
    }
  ...
  // Number of messages fetched this cycle — the activity signal that drives
  // adaptive poll backoff in pollSmalltoakLoop.
  return messages.length;
}

async function pollSmalltoakLoop(context) {
  let backoffMs = MIN_BACKOFF_MS;
  // Current adaptive interval, starting fast and widening while a room is idle.
  let idlePollMs = context.pollMs;

  while (!context.signal.aborted) {
    try {
      const fetched = await syncOnce(context);
      backoffMs = MIN_BACKOFF_MS;
      if (fetched > 0) {
        // Activity — snap back to the fast interval for responsive chat.
        idlePollMs = context.pollMs;
      } else {
        // Quiet — widen toward the idle cap to shed steady-state poll load.
        idlePollMs = Math.min(Math.round(idlePollMs * IDLE_BACKOFF_FACTOR), context.idlePollMs);
      }
      await sleep(idlePollMs, context.signal);
    } catch (error) {
      if (context.signal.aborted) return;
...
export async function runBridge({ chatId, file, transport, archive, cursorStore, signal }) {
  ...
  const context = {
    ...
    idlePollMs: Math.max(DEFAULT_IDLE_POLL_MS, DEFAULT_POLL_MS),
    persist,
    readReady,
    pendingWrites,
    selfInsertedContent: createSelfContentLedger(), // in-session content guard against watchFileLoop race
    existingFileContent: createSelfContentLedger(), // pre-existing file lines — skip re-append on fresh start
  };

  // When starting with no cursor (lastSmalltoakId=0) on a file that already has
  // content, polling from sinceId=0 replays all smalltoak history and would
  // re-append lines already in the file. Pre-load the file's current content
  // so syncOnce can skip any message whose decoded line is already there.
  if (state.lastSmalltoakId === 0) {
    try {
      const raw = await readFile(file, 'utf8');
      for (const line of raw.split('\n')) {
        if (line.trim()) context.existingFileContent.add(line);
      }
    } catch { /* new or unreadable file — ledger stays empty, normal append */ }
  }
