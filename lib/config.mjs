...
const MIRROR_STORE_DIR = resolve(homedir(), '.treebird-chat', 'rooms');
...
// tb-d21.2 security pass: the regex is a strict *allowlist* — separators (/ \),
// null bytes, unicode lookalikes (e.g. fullwidth solidus ／ U+FF0F), and encoded
// traversal are rejected by construction because they're outside the charset.
// `.`/`..` ARE inside the charset (dot is allowed), so they're excluded
// explicitly. A length cap stops a pathological chatId from forcing ENAMETOOLONG.
const SAFE_SEGMENT = /^[A-Za-z0-9._-]+$/;
const MAX_CHATID_LEN = 128; // generous for a room id; well under any FS NAME_MAX

function mirrorStorePath(chatId, storeDir) {
  if (
    typeof chatId !== 'string' ||
    chatId.length === 0 ||
    chatId.length > MAX_CHATID_LEN ||
    chatId === '.' ||
    chatId === '..' ||
    !SAFE_SEGMENT.test(chatId)
  ) {
    throw new Error(`unsafe chatId for mirror store: ${JSON.stringify(chatId)}`);
  }
  // Defense-in-depth: even if the allowlist above is ever loosened, refuse any
  // path that doesn't land directly inside storeDir. resolve() collapses any
  // traversal; the parent of the result MUST equal the resolved store dir.
  const root = resolve(storeDir);
  const path = resolve(root, `${chatId}.md`);
