...
export function findSessionByPath(filePath) {
  const sessions = readSessions();
  const entry = Object.entries(sessions).find(([, s]) => s.filePath === filePath);
...
export function resolvePublicUrl(rawUrl) {
  ...
  // Non-loopback but matches one of this machine's own IPs — surface other
  // interfaces as alternates so the invite block covers all reachable subnets.
