...
import { closeSync, constants, openSync, statSync, unlinkSync, writeSync } from 'node:fs';
...
// floods the chat with a multi-MB string. Truncation marker keeps the
// message visible-but-bounded. (/ts-review input_validation cap.)
...
export async function appendLines(filePath, agent, lines) {
  ...
  const safeLines = lines.map(l => {
    ...
    return collapsed.length > MAX_LINE_LEN
      ? collapsed.slice(0, MAX_LINE_LEN - TRUNCATION_MARKER.length) + TRUNCATION_MARKER
      : collapsed;
  });
