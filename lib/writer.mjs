...
import { UNVERIFIED_MARKER } from './watcher.mjs';
 ...
 * @param {{verified?: boolean}} opts - `verified: false` appends the
 *   display-only unverified marker to each line's body (SPEC §1 Option A).
 *   Omit/true: no marker (matches prior behavior for unaware callers).
 */
...
export async function appendLines(filePath, agent, lines, opts = {}) {
  const safeAgent = agent.replace(/[\r\n]/g, '');
  const t = nowHHMM();
  const marker = opts.verified === false ? UNVERIFIED_MARKER : '';
  ...
  const out = safeLines.map(l => `[${t} ${safeAgent}] ${l}${marker}`).join('\n') + '\n';
 ...
 */
export async function appendLine(filePath, agent, message, opts = {}) {
  return appendLines(filePath, agent, [message], opts);
}
